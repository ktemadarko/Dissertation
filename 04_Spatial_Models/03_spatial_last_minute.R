...
lmmodel <- lm(TFR~EDU + Pakistn + Bngldsh + Blck_Af + INC + LOGPDENS + SOCHO + DIV, data = SpatialData_SpatialPolygon)
summary(lmmodel)
WorkdayRatio
...
bptest.sarlm(durbinmodel, studentize = TRUE)
bptest.sarlm(durbinerrormodel, studentize = TRUE)
...
names(sums)
data.frame(sums$semat)

summary(impacts(lagmodel,tr=trMC,R=500),zstats=TRUE)


test1<-lapply(sums$sres, function(x){apply(x, 2, mean)})
test2<-lapply(sums$sres, function(x){apply(x, 2, sd)})
...
summary(impacts(durbinmodel,tr=trMC,R=500),zstats=TRUE)
...
summary(impacts(durbinerrormodel,tr=trMC,R=500),zstats=TRUE)
